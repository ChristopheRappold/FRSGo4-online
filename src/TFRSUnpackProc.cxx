...
TH1I* TFRSUnpackProc::MakeH1ISeries(const char* foldername,
                                    Int_t seriesnumber,
                                    Int_t crate,
                                    Int_t number,
                                    Bool_t remove) {
...
//	continue; // skip subevent SL
      ...
      switch(fInput->GetTrigger()) { // trigger value
      ...
      case 13: { // end of extraction, not used 
...
case 15: { //proc ID=15 - TimeStamp
	 ...
	 // std::cout<<"length = "<<lenMax<<std::endl;
      ...
      for (int i = 0; i < 32; ++i) {
		...
		if(getbits(*pdata,2,12,5)==16&&len<lenMax) {//EOB
			pdata++; len++;
		}
	}
	  break;
	  ...
	  case 20: {
         UnpackUserSubevent(psubevt, tgt); //for historical reasons
	  }
