...
void TFRSAnlProc::Create_MUSIC_Data() {
...
void TFRSAnlProc::Create_SCI_Data() {
...
void TFRSAnlProc::Create_ID_Data() {
...
void TFRSAnlProc::Create_SI_Data() {
}
void TFRSAnlProc::Create_CT_Data() {
}
 ...
 void TFRSAnlProc::Procceed_MUSIC_Analysis(TFRSSortEvent& srt, TFRSCalibrEvent& clb, TFRSAnlEvent& tgt) 
 {
   ...
   // Munich MUSIC 
     ...
     for (int i=0;i<8;i++) { /* 8 anodes of TUM MUSIC */		
	 ...
	 if(bDrawHist) 
	   hMUSIC1_E[i]->Fill(srt.music_e1[i]);
         ...
         if (tgt.music_b_e1[i]) tgt.music1_anodes_cnt++;
       }
...
//       /****** second MUSIC *****/
     ...
     for (int i=0;i<4;i++) { /* 4 anodes of MUSIC OLD */		
       /****** third MUSIC *****/
       if ( srt.music_e3[i] > 0) { 
 			if(bDrawHist) 
         		hMUSIC3_E[i]->Fill(srt.music_e3[i]);
         tgt.music_b_e3[i] = cMusic3_E[i]->Test(srt.music_e3[i]);
         if (tgt.music_b_e3[i]) tgt.music3_anodes_cnt++;
       }
       if (srt.music_t3[i] > 0) { 
 			if(bDrawHist) 
...
//      Float_t r4 = (srt.music_e1[6] - music->e1_off[6])*(srt.music_e1[7] - music->e1_off[7]);
     ...
     if (tgt.music3_anodes_cnt == 4) {         // OLD MUSIC
 	...
 	if(bDrawHist) hMUSIC3_dE->Fill(tgt.de[2]);
       } 
...
//     }
 ...
 void TFRSAnlProc::Procceed_SCI_Analysis(TFRSSortEvent& srt, TFRSCalibrEvent& clb, TFRSAnlEvent& tgt) 
 {
   ...
   TSCIParameter* sci = (TSCIParameter*) GetParameter("SCIPar");
   ...
   if (tgt.sci_b_tofll2 && tgt.sci_b_tofrr2) {      /* TOF SC41 - SC21 [ps]  */
...
{
  TFRSParameter* frs = (TFRSParameter*) GetParameter("FRSPar");
  TIDParameter* id = (TIDParameter*) GetParameter("IDPar");
  ...
  if(id->x2_select == 0) 
    if(tgt.sci_b_x[2]) tgt.id_x2 = tgt.sci_x[2]; /* pos index 2 = SC21 */

  if(id->x2_select == 1) {
     
    tgt.id_x2=999;
    if(clb.b_tpc_xy[2]&&clb.b_tpc_xy[3])tgt.id_x2 = clb.tpc_x_s2_foc;
    if(clb.b_tpc_xy[2]&&!clb.b_tpc_xy[3])tgt.id_x2 = clb.tpc_x[2];
    if(!clb.b_tpc_xy[2]&&clb.b_tpc_xy[3])tgt.id_x2 = clb.tpc_x[3];
    if(!clb.b_tpc_xy[2]&&!clb.b_tpc_xy[3])tgt.id_x2 = tgt.sci_x[2];
 ...
 /****  S4  (MUSIC OLD)   */
  ...
  if((tgt.de[2]>0.0) && (tgt.id_beta>0.0) && (tgt.id_beta<1.0)) {
    ...
    if ((tgt.id_z3>0.0) && (tgt.id_z3<100.0)) {
      tgt.id_b_z3 = kTRUE;
      if(bDrawHist)  hID_Z3->Fill(tgt.id_z3);
    }
    if(bDrawHist)  hID_Z_Z3->Fill(tgt.id_z,tgt.id_z3);
  }
...
//        tgt.id_b_z_AoQ[i] = cID_Z_AoQ[i]->Test(tgt.id_AoQ, tgt.id_z);      
        ...
        if(i==0){  // up to 5 Aug it was [0] now [4]
